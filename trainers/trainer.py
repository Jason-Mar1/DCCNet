...
from utils.misc import AverageMeter, DictAvgMeter, get_current_datetime, easy_track, load_model_checkpoint
...
class Trainer(object):
    ...
    def load_ckpt(self, model, path):
        if path is not None:
            ...
            report = load_model_checkpoint(model, path, self.device, allow_partial=False)
            self.log(
                'Checkpoint compatibility: missing={}, unexpected={}'.format(
    ...
    def test(self, model, test_dataloader, checkpoint=None):
        ...
        if checkpoint is None:
            raise ValueError('Testing requires a checkpoint. Set checkpoint in the config or pass --checkpoint.')
        self.load_ckpt(model, checkpoint)
        ...
        for key, value in result_meter.avg.items():
            if key == 'mae':
                ...
                self.log('{}: {:.4f}'.format(key, sqrt(value)), end=' ')
        self.log('')
    ...
    def vis(self, model, test_dataloader, checkpoint=None):
        ...
        if checkpoint is None:
            raise ValueError('Visualization requires a checkpoint. Set checkpoint in the config or pass --checkpoint.')
        self.load_ckpt(model, checkpoint)
