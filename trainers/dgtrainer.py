...
class DGTrainer(Trainer):
    def __init__(self, seed, version, device, log_para, patch_size, mode, lambda_aux=0.1, lambda_con=0.01):
        ...
        if self.lambda_aux < 0 or self.lambda_con < 0:
            raise ValueError("lambda_aux and lambda_con must be non-negative.")
    ...
    def train_step(self, model, loss, optimizer, batch, epoch):
        ...
        if self.mode == 'simple':
            ...
            loss_den = self.compute_count_loss(loss, dmaps1, gt_datas) + self.compute_count_loss(loss, dmaps2, gt_datas)
            loss_total = loss_den + self.lambda_con * loss_con
            loss_total.backward()
            optimizer.step()
            ...
            loss_total = loss_den + self.lambda_aux * loss_cls
            loss_total.backward()
            optimizer.step()
            ...
            loss_den = self.compute_count_loss(loss, dmaps1, gt_datas) + self.compute_count_loss(loss, dmaps2, gt_datas)
            loss_aux = F.binary_cross_entropy(cmaps1, gt_cmaps) + F.binary_cross_entropy(cmaps2, gt_cmaps)
            # Paper Eq. (12): L_den + lambda_aux L_aux + lambda_con L_con.
            # These coefficients come from the release YAML, never a hidden
            # trainer constant.
            loss_total = loss_den + self.lambda_aux * loss_aux + self.lambda_con * loss_con
