...
class DenClsDataset(DensityMapDataset):
    ...
    def __init__(self, root, crop_size, downsample, method, is_grey, unit_size, pre_resize=1,
                 roi_map_path=None, gt_dir=None, gen_root=None, foreground_threshold=0.4):
        ...
        # The first view is the ordinary normalized image inherited from
        # ``BaseDataset``.  The second view changes appearance only: ColorJitter
        # followed by actual additive Gaussian noise, not Gaussian blur.
    ...
    def _annotation_path(self, img_fn):
        image_path = Path(img_fn)
        annotation_stem = image_path.stem
        ...
        try:
            is_source_image = os.path.commonpath(
                [os.path.abspath(str(image_path)), os.path.abspath(self.root)]
            ) == os.path.abspath(self.root)
        except ValueError:
            # Different Windows drives cannot have a common path; generated
            # samples follow the historical ``<root>/train/<stem>.npy`` route.
            is_source_image = False

        if is_source_image:
            return image_path.with_name(annotation_stem + ".npy")
        return Path(self.root) / "train" / (annotation_stem[:-2] + ".npy")

    def __getitem__(self, index):
        img_fn = self.img_fns[index]
        img, _ = self._load_img(img_fn)
        basename = Path(img_fn).stem
        gt_path = self._annotation_path(img_fn)
        ...
        if self.method == 'train':
            if self.gt_dir is None:
                ...
                dmap_path = Path(self.gt_dir) / (gt_path.stem + ".npy")
    ...
    def _train_transform(self, img, gt, dmap):
        ...
        if factor != 1.0:
            ...
            if min(new_w, new_h) >= min(self.crop_size[0], self.crop_size[1]):
                ...
                img = img.resize((w, h))
                dmap_sum = dmap.sum()
                dmap = F.resize(dmap, (h, w), antialias=True)
    ...
    def _val_transform(self, img, gt, name):
        ...
        # Post-processing
        img1 = self.transform(img)
        # Evaluation must be deterministic: both outputs are the ordinary
        # view, rather than sampling the training-only appearance augmentation.
        img2 = img1.clone()
        gt = torch.from_numpy(gt.copy()).float()
