...
class JHUDomainDataset(Dataset):
    ...
    def __init__(self, root, domain_label, crop_size, downsample, method,
        ...
        phase = {'train': 'train', 'val': 'val', 'test': 'test'}.get(method)
        if phase is None:
            raise ValueError("method must be one of: train, val, test")
        manifest_dir = Path(split_root) if split_root else Path(root) / "domains"
        manifest = manifest_dir / f"{domain_label}_{phase}.txt"
        if not manifest.is_file():
            raise FileNotFoundError(
                f"Missing JHU domain manifest: {manifest}. Expected one relative image path per line."
            )
        with manifest.open("r", encoding="utf-8") as handle:
            entries = [line.strip() for line in handle if line.strip() and not line.lstrip().startswith("#")]
        self.img_fns = [
            entry if os.path.isabs(entry) else str(Path(root) / phase / entry)
    ...
    def __getitem__(self, index):
        ...
        gt_path = image_path.with_suffix('.npy')
