...
def run_qnrf(origin_dir, save_dir, min_size, max_size, split_dir):
    for phase in ['Train', 'Test']:
        ...
        if phase == 'Train':
            ...
            for sub_phase in sub_phase_list:
                ...
                if not os.path.exists(sub_save_dir):
                    os.makedirs(sub_save_dir)
                split_list = _read_split(Path(split_dir) / 'ucf_qnrf' / f'{sub_phase}.txt')
                for image_name in split_list:
                    im_path = os.path.join(sub_dir, image_name)
                    name = os.path.basename(im_path)
...
def run_sta(origin_dir, save_dir, min_size, max_size, split_dir):
    if os.path.basename(os.path.normpath(origin_dir)).lower() == 'part_a':
        part = 'part_a'
    else:
        part = 'part_b'
    for phase in ['train', 'test']:
        ...
        if phase == 'train':
            ...
            train_list = _read_split(Path(split_dir) / 'shanghaitech' / f'{part}_train.txt')
            val_list = _read_split(Path(split_dir) / 'shanghaitech' / f'{part}_val.txt')
...
def parse_args():
    ...
    parser.add_argument('--origin-dir', default='jhu_crowd_v2.0',
                        help='original data directory')
    parser.add_argument('--data-dir', default='data/jhu_crowdpp',
    ...
    parser.add_argument('--dataset', default='jhu', type=str,
                        ...
                        help='dataset name')
    parser.add_argument('--split-dir', default=str(PROJECT_ROOT / 'splits'),
                        help='Repository split directory (used by qnrf, sta, and stb).')
...
if __name__ == '__main__':
    ...
    if dataset == 'jhu':
        ...
        run_qnrf(origin_dir, save_dir, min_size, max_size, split_dir)
    elif dataset == 'smartcity':
        run_smartcity(origin_dir, save_dir, min_size, max_size)
    elif dataset == 'sta' or dataset == 'stb':
        run_sta(origin_dir, save_dir, min_size, max_size, split_dir)
    elif dataset == 'cc50':
